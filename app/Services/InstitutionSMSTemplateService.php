<?php 
    ...
    class InstitutionSMSTemplateService {
        ...
        public function getSmsModuleDetails($allSessions) {
            ...
            foreach($smsModuleDetails['smsModules'] as $module) {
                ...
                foreach($smsFor as $data) {
                    ...
                    if($data != 'MESSAGE_CENTER') {
                        $details = $institutionSMSTemplateRepository->fetch($data, $allSessions);
                        if($details){
                            $templateId = $details->sms_template_id;
                            $senderId = $details->sender_id;
                            $smsTemplateDetails = $SMSTemplateRepository->getSmsTemplatesUsingSenderId($senderId, $allSessions);
                        ...
                        if(!empty($institutionSenderId)) {
                            $smsTemplateDetails = $SMSTemplateRepository->getSmsTemplatesUsingSenderId($institutionSenderId, $allSessions);
                            $smsModuleDetails['mc_sms_template_details'][] = $smsTemplateDetails;
                        }
                        $messageDetails = $institutionSMSTemplateRepository->fetchDetails($data, $allSessions);
        ...
        public function add($smsTemplateData, $allSessions){
            $institutionSMSTemplateRepository = new InstitutionSMSTemplateRepository();
            $smsTemplateDetails = '';
            $institutionRepository = new InstitutionRepository();
            
            $institutionId = $smsTemplateData->id_institute;
            $smsModuleDetails = $this->getSmsModuleDetails($allSessions);
