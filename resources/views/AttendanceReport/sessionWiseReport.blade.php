        ...
        <div class="row">
            <div class="col-lg-12">
                <table class="table" id="tblData" style="border-collapse:collapse;border:1px solid;">
                    <thead>
                        <tr><th colspan="{{ (count($getReportData['dateList']) * count($getReportData['sessions']) ) + 7 }}" class="text-center fw-500"><h3>{{ $institute->name }}</h3></th></tr>
                        <tr><th colspan="{{ (count($getReportData['dateList']) * count($getReportData['sessions']) ) + 7 }}" class="text-center fw-500">{{ $institute->address }}, {{ $institute->post_office }}, {{ $institute->pincode }}</th></tr>
                        <tr><th colspan="{{ (count($getReportData['dateList']) * count($getReportData['sessions']) ) + 7 }}" class="text-center fw-500">Sessionwise Report</th></tr>
                        <tr><th colspan="{{ (count($getReportData['dateList']) * count($getReportData['sessions']) ) + 7 }}" class="text-center fw-500">Generated Time : @php echo date('d/m/Y H:i:s'); @endphp</th></tr>
                        <tr rowspan="2">
                            ...
                            @foreach($getReportData['dateList'] as $date)
                                <th class="fw-500" class="text-center" colspan="{{ count($getReportData['sessions']) }}">{{ $date }}</th>
                            @endforeach                            
