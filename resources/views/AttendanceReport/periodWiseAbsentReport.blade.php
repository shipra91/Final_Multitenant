        ...
        <div class="row">
            <div class="col-lg-12">
                <table class="table" id="tblData" style="border-collapse:collapse;border:1px solid;">
                    <thead>
                        <tr><th colspan="{{ count($getReportData['periods']) + 5 }}" class="text-center fw-500"><h3>{{ $institute->name }}</h3></th></tr>
                        <tr><th colspan="{{ count($getReportData['periods']) + 5 }}" class="text-center fw-500">{{ $institute->address }}, {{ $institute->post_office }}, {{ $institute->pincode }}</th></tr>
                        <tr><th colspan="{{ count($getReportData['periods']) + 5 }}" class="text-center fw-500">Periodwise Absent Report</th></tr>
                        <tr><th colspan="{{ count($getReportData['periods']) + 5 }}" class="text-center fw-500">Generated Time : @php echo date('d/m/Y H:i:s'); @endphp</th></tr>
                        <tr rowspan="2">
                            ...
                            <th class="fw-500" class="text-center" colspan="{{ count($getReportData['periods']) }}">{{ $getReportData['date'] }}</th>
                            <th class="fw-500" rowspan="2">CONTACT</th>
                        </tr>  
                                        
                        <tr>
                            @foreach($getReportData['periods'] as $period)                                  
                                <th class="fw-500">{{ $period['name'] }}</th>                                
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($getReportData['studentDetailArray'] as $index => $reportData)
                            @if($reportData['studentDetail'])
                                ...
                                <tr>
                                    ...
                                    @foreach($getReportData['periods'] as $period)      
                                        <td>{{ $reportData['attendanceDetail'][$getReportData['date']][$period['id']] }}</td>                               
                                    @endforeach
