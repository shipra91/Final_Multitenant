                        ...
                        <div class="card">
                            ...
                            <div class="card-content">
                                ...
                                <form method="POST" id="templateForm" enctype="multipart/form-data">
                                    <input type="hidden" id="id_template" value="{{ $templateData->id }}" />
                                    <div class="row">
                                        <div class="col-lg-4">
                                            <div class="form-group">
                                                ...
                                                <select class="selectpicker" name="template_category" id="template_category" data-size="4" data-style="select-with-transition" data-live-search="true" title="Select" required="required" data-parsley-errors-container=".templateCategory">
                                                    @foreach($templateCategory as $category)
                                                        <option value="{{ $category['label'] }}" @if($category['label'] == $templateData->template_category) {{ "selected" }} @endif>{{ $category['display_name'] }}</option>
                                                    @endforeach
                                                </select>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="template_name" class="form-control" value="{{ $templateData->template_name }}" required />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="form-group">
                                                <label class="control-label mt-0 form-group mb-8">Template Description<span class="text-danger">*</span></label>
                                                <textarea class="ckeditor" name="template_description" required>{{ $templateData->template_content }}</textarea>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    $(document).ready(function(){
        ...
        $("#single_token_type, #manual_entry_tokan").change(function(event){
            ...
            var tag_keyword = $(this).val(); console.log(tag_keyword);
        ...
        $('body').delegate('#templateForm', 'submit', function(e){
            ...
            if ($('#templateForm').parsley().isValid()){
                ...
                $.ajax({
                    url:"{{ url('/etpl/dynamic-template') }}/"+id,
                    type:"post",
                    dataType:"json",
