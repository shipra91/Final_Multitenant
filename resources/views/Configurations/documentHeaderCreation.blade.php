                    ...
                    <div class="col-md-12 col-md-offset-0">
                        <div class="card">
                            ...
                            <div class="card-content">
                                <h4 class="card-title">Add Document Header</h4>
                                <form method="POST" id="documentHeaderForm" action="#">
                                    <div id="repeater">
                                        ...
                                        <div class="row" id="section_1" data-id="1">
                                            <div class="col-lg-4 col-lg-offset-0">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" name="documentHeader[]" id="documentHeader_1" required />
                                                </div>
                                            </div>
                                    ...
                                    <div class="row pull-right">
                                        <div class="col-lg-12 col-lg-offset-0 form-group">
                                            ...
                                            <a href="{{url('/etpl/document-header')}}" class="btn btn-finish btn-fill btn-wd btn btn-danger">Close</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click', '#add_more', function(){
            ...
            html += '<label class="control-label">Document Header<span class="text-danger">*</span></label>';
            html += '<input type="text" class="form-control" name="documentHeader[]" id="documentHeader_'+count+'" required />';
            html += '</div>';
            html += '</div>';
        ...
        $("#documentHeaderForm").parsley({
            triggerAfterFailure: 'input change focusout changed.bs.select'
        });

        // Save document header
        $('body').delegate('#documentHeaderForm', 'submit', function(e){
            e.preventDefault();

            var btn=$('#submit');

            if($('#documentHeaderForm').parsley().isValid()){

                $.ajax({
                    url:"/etpl/document-header",
                    type:"post",
                    dataType:"json",
                        ...
                        if(result['status'] == "200"){
                            ...
                            if(result.data['signal'] == "success"){
                                ...
                                swal({
                                    ...
                                    window.location.replace('/etpl/document-header');
                                }).catch(swal.noop)
